...
/* Only define these if the compiler doesn't provide <stdint.h> natively.
   MSVC 2010+ (1600+) already defines int16_t, int32_t etc. via <cstdint>. */
...
#else
/* MSVC 2010+ already has int16_t/int32_t via <stdint.h>/<cstdint>,
   but u_int*_t are non-standard - define only those. */
#include <stdint.h>
 ...
 *******************************************************/
/*
 * DBTYPE -- Berkeley DB access-method type constants.
 * oledb.h (pulled in by MFC) already typedef's WORD DBTYPE;
 * only define our own typedef when oledb.h has NOT been included.
 */
/* DBTYPE -- BerkeleyDB access-method type.
   On Windows, DBTYPE may already be typedef'd by OLE DB / DAO headers
   (as WORD).  We skip our typedef in that case and just use integer
   constants -- the BDB code only ever compares DBTYPE values, so
   the underlying integer width difference is harmless. */
#ifndef _BDB_DBTYPE_
#define _BDB_DBTYPE_
#if !defined(_INC_OLE) && !defined(__oledb_h__) && !defined(__AFXDAO_H) && !defined(_OLEDB_H_) && !defined(DBDAOINT_H_INCLUDED)
