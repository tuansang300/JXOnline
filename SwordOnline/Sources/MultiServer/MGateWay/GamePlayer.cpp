...
{
	TStreamData *pBlock = m_theRoleStreamVein.Block_IN( reinterpret_cast< const KBlock * >( pData ) );
...
void CGamePlayer::InitEventProcessor()
{
	...
	EventProcessor[enumQuitEvent] = &CGamePlayer::Quit;
	EventProcessor[enumWorkEvent] = &CGamePlayer::Work;
}
...
bool CGamePlayer::VerifyAccount()
{
	...
	m_pPlayerServer->SendPackToClient( m_lnIdentityID );
	...
	if ( NULL != ( pData = m_theDataQueue.Top( PROTOL_GETROLELIST ) ) )
	{
		...
		if ( nRoleCount > 0 && nRoleCount <= enumRoleListCount && pRoleList )
		{
			...
			for ( int no = 0; no < nRoleCount; no ++ )
			{
				...
				Trace( ToString( m_lnIdentityID ), m_theRoleDBList[no].szName );
			}
			
			/*
			 * Send this info to player
			 */
			KRoleBlockProcess theRoleBlocks( const_cast< TStreamData * >( ( const TStreamData * )pData ), datalength );
	...
	case enumQueryRoleInfo:
		{
			...
			// TEMPORAL
			_tstring	 sRoleName = m_theRoleDBList[ m_theRoleInfomation.nIndexInList ].szName;
