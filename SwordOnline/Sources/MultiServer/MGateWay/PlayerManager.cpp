...
bool CPlayerManager::StartupNetwork()
{
	...
	{
		pClientFactory->SetEnvironment( g_scnDBRoleBufferSize );
...
int CPlayerManager::Helper()
{
	while ( WAIT_OBJECT_0 != ::WaitForSingleObject( m_hQuitEvent, 0L ) )
	{
		...
		while( pData && dataLength )
		{
			...
			/* Extract ulIdentity from the TStreamData embedded in the block payload.
			   The first bytes of pDataBuffer are the serialized TStreamData header. */
			TStreamData *pStreamHdr = reinterpret_cast< TStreamData * >( pBlock->pDataBuffer );
			UINT uID = pStreamHdr->ulIdentity;
	...
	case c2s_login:
		{
			...
			m_thePlayers[index]->Create( pLAI->Account, pLAI->Password.szPassword );
