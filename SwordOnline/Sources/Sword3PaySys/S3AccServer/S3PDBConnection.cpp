...
{
	m_iDBIdentifier = iDBIdentifier;

	// Read database connection info from INI / config
	// and call OpenConnect on the underlying S3PDB_MSSQLServer_Connection
	...
	strcat(szIniPath, "S3AccServer.ini");

	char szSection[256];
	switch (iDBIdentifier)
	{
	case def_CARDDB:
		strcpy(szSection, "card");
		break;
	case def_ACCOUNTDB:
		strcpy(szSection, "account");
		break;
	case def_ROLEDB:
		strcpy(szSection, "role");
		break;
	default:
		strcpy(szSection, "account");
		break;
	...
	char szBuffer[256];
	GetPrivateProfileStringA(szSection, "Server",   "", szBuffer, sizeof(szBuffer), szIniPath);
	dbInfo.strServer = szBuffer;
	GetPrivateProfileStringA(szSection, "DataBase", "", szBuffer, sizeof(szBuffer), szIniPath);
	dbInfo.strDataBase = szBuffer;
	GetPrivateProfileStringA(szSection, "User",     "", szBuffer, sizeof(szBuffer), szIniPath);
	dbInfo.strUser = szBuffer;
	GetPrivateProfileStringA(szSection, "PassWord", "", szBuffer, sizeof(szBuffer), szIniPath);
	dbInfo.strPassword = szBuffer;

	if (dbInfo.strServer.empty() || dbInfo.strDataBase.empty())
		return FALSE;

	return OpenConnect(&dbInfo) ? TRUE : FALSE;
}
