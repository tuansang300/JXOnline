...
#define LEFT_TIME		60000	//�������ߺ�ĵȴ�ʱ��
#define PING_TIME		20000	//�����ȴ��ͻ���Ping��ʱ��
#define SENDPING_TIME	0x7fffffff	//��ͻ��˷���Ping��ʱ��
...
void KGatewayDataProcess::AutoTime()
{
	...
	{
		if ((nNow - i->second) / 1000 >= 1)	//ÿ��ѭ������1���Ѿ���ʱ���û�
		{
			m_UserNames.erase(i);
			break;
		}
	...
	if (m_nLeftStatusTime > 0 && (nNow - m_nStatusTime > m_nLeftStatusTime) && IsWorkAgain())
	{
		...
		if (pConn)
		{
			S3PAccount::ElapseAll(pConn, m_nGameID);	//ֻ��Ǯ,������
			SetStatus(gdp_free);
			return;
		}
	}
	...
	if (nProtocol == c2s_gatewayverify || nProtocol == c2s_gatewayverifyagain)
	{
		...
		else
		{
			...
			else
			{
				...
				if (bRet)
				{
					if (nProtocol == c2s_gatewayverify)	//gatewayverify�ɹ���Ҫ����״̬�����ֳ�
					{
...
BOOL KGatewayDataProcess::ProAccountLogin(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLoginInfo))
	{
			...
			)
		{
			...
			if (m_UserNames.find(*((UserName*)pAccInfo->Account)) == m_UserNames.end())	//���ں�������
			{
...
DWORD KGatewayDataProcess::Main(LPVOID lpParam)
{
	...
	{
		S3PAccount::ElapseAll(m_pConn, m_nGameID);	//ֻ��Ǯ,������
	}
...
BOOL KGatewayDataProcess::ConnectErrorWork(unsigned long nID, unsigned long Address, short Port)
{
	...
	if (m_Status == gdp_verify ||	//ֻ���⼸��״̬�µ�m_Address��Ч
		m_Status == gdp_work ||
		m_Status == gdp_verifyagain ||
		m_Status == gdp_errorconnect
		)
	{
		if (m_Address == Address)	//���Ѿ����ӵ�Address
		{
			bRet = TRUE;
		}
		goto exit0;
...
BOOL KGatewayDataProcess::OutofWork(unsigned long nID)
{
	...
	if (m_nConnectID == nID)
	{
		...
		if (m_Status == gdp_work)
		{
			m_Status = gdp_again;
			m_nLeftStatusTime = LEFT_TIME;
			m_nStatusTime = GetTickCount();	//��ʣ��ʱ������,Ϊ�˿ͻ�����������
...
void S3PDBSocketPool::Process(void* pSocket, const IBYTE* pData, size_t datalength)
{
	// Legacy static entry point - data processing is now handled
	// by KGatewayDataProcess::Main() threads directly
}