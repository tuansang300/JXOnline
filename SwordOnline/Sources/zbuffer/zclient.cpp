...
//select���з��ͺͽ���
	...
	time_out.tv_sec = 0;
	time_out.tv_usec = 2;
	//char temp_buffer[MAX_TEMP_SIZE];														//64k����ʱ������
	int length = 0;
	while(true) {
		if(parent->bStop) 
			break;
		...
		if (select(parent->nSocket + 1, &read_set, NULL, NULL, &time_out) != 0) {					//�����ݵȴ���
			int bufsize = 0;
...
//					length = length;
		...
		if(send_ptr) {
			...
			//printf("send data %d\n", length);
			if(select(parent->nSocket + 1, NULL, &write_set, NULL, &time_out) != 0) {					//�����ݵȴ�д
