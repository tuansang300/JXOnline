...
static unsigned gs_holdrand = (unsigned)time(NULL);
...
void *work_thread(LPVOID id) {
	...
	while(true) {
		...
		if(!bIORet && dwIOError != WAIT_TIMEOUT ) {
			...
			parent->buffer->clear(client->read_index, false);
			parent->buffer->clear(client->write_index, true);
...
void IServer::closeConnection(client_info *client) {
	...
	closesocket(client->sock);
	buffer->clear(client->read_index, false);
	buffer->clear(client->write_index, true);
	client->sock = INVALID_SOCKET;
...
//ƽ̨��ش���,ʵ�ʷ����ص���IO����,��������---------------------------------------
	...
	if(client->m_wsaOutBuffer.buf) {
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) {		//���ִ���
...
int IServer::PreparePackSink() {		//׼����ʼ���ݷ�װ
	return 0;
}
...
int IServer::ShutdownClient(const unsigned long &ulnClientID) { 
	if(clients[ulnClientID].sock != INVALID_SOCKET) {
		...
		buffer->clear(clients[ulnClientID].read_index, false);
		buffer->clear(clients[ulnClientID].write_index, true);
...
const char * IServer::GetClientInfo(const unsigned long &ulnClientID) {
	static char info[64];
	if(ulnClientID < (unsigned long)max_client && clients[ulnClientID].sock != INVALID_SOCKET) {
		struct sockaddr_in *addr_in = (struct sockaddr_in *)&clients[ulnClientID].addr;
		sprintf(info, "%s:%d", inet_ntoa(addr_in->sin_addr), ntohs(addr_in->sin_port));
		return info;
	}
	return NULL;
}
